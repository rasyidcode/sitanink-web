        ...
        <div class="col-xs-12">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">Edit Lokasi Kerja</h3>
                </div>
                <div class="box-body">
                    <div class="row">
                        <div class="col-xs-12 col-sm-offset-1 col-sm-8 col-md-offset-1 col-md-8 col-lg-offset-4 col-lg-4">
                            <form id="form-edit-lokasi-kerja" action="<?= route_to('lokasi-kerja.update', $lokasiKerjaData->id) ?>" method="POST" role="form">
                                <?= csrf_field() ?>
                                <div class="row">
                                    <div class="col-xs-12">
                                        <div class="form-group <?= isset($fdErr['nama']) ? 'has-error' : '' ?>">
                                            <label class="control-label"><?= isset($fdErr['nama']) ? $errIcon : '' ?>&nbsp;Nama Lokasi</label>
                                            <input type="text" name="nama" class="form-control" placeholder="Masukkan nama lokasi ..." value="<?= $lokasiKerjaData->nama ?>">
                                ...
                                <div class="row">
                                    <div class="col-xs-12">
                                        <div class="form-group <?= isset($fdErr['lon']) ? 'has-error' : '' ?>">
                                            ...
                                            <p>Koordinat : <strong id="koordinat"><?= $lokasiKerjaData->lat . ',' . $lokasiKerjaData->lon ?></strong></p>
...
<script>
    $(function() {
        mapboxgl.workerUrl = "<?=site_url('assets/js/mapbox-gl-csp-worker.js')?>";
        mapboxgl.accessToken = 'pk.eyJ1IjoiamFtaWxjaGFuIiwiYSI6ImNrNG1oOWI1YjJqNmUzZG9iZjU3MHRhYzQifQ.U4Y11hHODjokNS7Jmlw0Xg';
        var lon = '<?= $lokasiKerjaData->lon ?>';
        var lat = '<?= $lokasiKerjaData->lat ?>';
        var map = new mapboxgl.Map({
            container: 'lokasi-kerja-peta', //id elemen html
            style: 'mapbox://styles/mapbox/streets-v11',
            center: [lon, lat], //koordinat lokasi garis bujur dan lintang,longitude dan latitude
        ...
        map.on('click', function(e) {
            ...
            $('#form-edit-lokasi-kerja').find('input[name="lat"]').val(lk.lat);
            $('#form-edit-lokasi-kerja').find('input[name="lon"]').val(lk.lng);
        });
    });
</script>
<?= $renderer->endSection() ?>
